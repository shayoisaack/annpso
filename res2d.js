...
function Res(gridblocksX, gridblocksY) {
    ...
    this.maxPP = function(){//calculate the maximum productivity potential for use in potential maps
        let maxPP = 0;
        for(let i = 0; i < this.rows; i++){
            for(let j = 0; j < this.cols; j++){
...
function restoreResMethods(res){
    ...
    res.addWells = function(wells) {
        ...
        for (let wellIndex = 0; wellIndex < wells.length; wellIndex++) {
            let loc = wells[wellIndex].loc;
            //if(loc.x < 1 || loc.y < 1 || loc.x > res.rows - 2 || loc.y > res.cols - 2) continue;
            ...
            if (wells[wellIndex].condition === 'pressure' || wells[wellIndex].condition === undefined) {
                ...
                if(this.cell[loc.x][loc.y].qo_ < 0) this.cell[loc.x][loc.y].qo_= 0;// *= -1;
                if(this.cell[loc.x][loc.y].qw_ < 0) this.cell[loc.x][loc.y].qw_ = 0;//*= -1;
...
exports.Res = Res;
exports.restoreResMethods = restoreResMethods;